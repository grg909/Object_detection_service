...
def main():
    ...
    # Load data
    origin_files, label_names = get_cocoapi('coco2017', 'train&val', 'train2017')
    num_labels = len(label_names)

    train_data_pd = origin_files.sample(frac=0.2, random_state=2019)
    val_data_pd = origin_files.drop(train_data_pd.index)
    val_data_pd = val_data_pd.sample(frac=0.02, random_state=9102)
    ...
    dataloaders = {x: DataLoader(datasets[x], batch_size=4, shuffle=True, num_workers=12)
                  for x in ['train', 'val']}

    model = Resnet_Model(num_labels=num_labels).to(device)
    optimizer = optim.SGD(model.fc.parameters(), lr=0.001, momentum=0.9)
    optimizer_adam = optim.Adam(model.fc.parameters(), lr=1e-4, amsgrad=True, weight_decay=1e-4)
